 ...
 */
class CacheAdapter extends BaseAdapter_1.BaseAdapter {
    name = 'cache';
    capabilities = {
        persistent: true,
        synchronous: false,
    ...
    async openCache() {
        if (this.cache)
            return this.cache;
        if (!('caches' in window)) {
            throw new errors_1.NotSupportedError('Cache API not available');
        }
    ...
    async set(key, value) {
        ...
        try {
            ...
            if (this.isQuotaError(error)) {
                throw new errors_1.QuotaExceededError('Cache quota exceeded', { key, size: (0, utils_1.getObjectSize)(value) });
            }
            throw new errors_1.StorageError(`Failed to set key ${key} in cache: ${error}`);
        }
    }
    /**
