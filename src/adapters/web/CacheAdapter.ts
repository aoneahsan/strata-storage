...
import { getObjectSize, serialize, deserialize } from '@/utils';
...
export class CacheAdapter extends BaseAdapter {
  ...
  async get<T = unknown>(key: string): Promise<StorageValue<T> | null> {
    ...
    try {
      ...
      const data = deserialize(await response.text()) as StorageValue<T>;
  ...
  async remove(key: string): Promise<void> {
    ...
    if (this.hasChangeListeners()) {
      ...
      if (response) {
        try {
          oldValue = deserialize(await response.text()) as StorageValue;
        } catch {
          oldValue = null;
        }
      }
