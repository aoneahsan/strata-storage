 ...
 */
class CookieAdapter extends BaseAdapter_1.BaseAdapter {
    name = 'cookies';
    capabilities = {
        persistent: true,
        synchronous: false,
    ...
    async set(key, value) {
        ...
        try {
            ...
            if (encoded.length > this.maxCookieSize) {
                throw new errors_1.QuotaExceededError(`Cookie size ${encoded.length} exceeds limit ${this.maxCookieSize}`, { key, size: encoded.length });
            }
            ...
            throw new errors_1.StorageError(`Failed to set cookie ${key}: ${error}`);
        }
    }
    /**
...
//# sourceMappingURL=CookieAdapter.js.map