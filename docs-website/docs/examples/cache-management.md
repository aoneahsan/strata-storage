...
class CacheManager {
  ...
  async invalidate(pattern: string | RegExp) {
    // query() matches stored values, not the storage key — filter keys and
    // remove them (or use this.storage.clear({ pattern }) to drop them at once).
    const re = typeof pattern === 'string' ? new RegExp(pattern) : pattern;
    const keys = (await this.storage.keys()).filter((k) => re.test(k));
...
class SmartCacheInvalidation {
  ...
  async invalidateTag(tag: string) {
    // Tags passed to set(key, value, { tags }) are record metadata — query()
    // can't see them. Use clear({ tags }) to drop every entry carrying the tag.
    await this.storage.clear({ tags: [tag] });
  }
}
```
