...
const cache = new Strata({
  // Preference order: try memory first, then persistent stores
  defaultStorages: ['memory', 'localStorage', 'indexedDB'],
  ttl: {
...
### Web Browser
```typescript
// Prefer high-capacity stores; fall back to smaller ones if unavailable
const webCache = new Strata({
  defaultStorages: ['cache', 'indexedDB', 'localStorage']
});
```
...
const mobileCache = new Strata({
  ...
  ttl: {
    autoCleanup: true,
    cleanupInterval: 300000, // 5 minutes — larger interval is battery-friendly
    batchSize: 50            // cap work per cleanup cycle
  }
});
```
...
const serverCache = new Strata({
  // memory first, with a persistent fallback
  defaultStorages: ['memory', 'sqlite'],
  ttl: { defaultTTL: 600000, autoCleanup: true }
});
```
