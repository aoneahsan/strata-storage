...
interface SyncConfig {
  ...
  interval?: number;          // Background sync interval (ms)
...
> The storage-event fallback is **localStorage-only** (cross-tab); it does not
> claim `sessionStorage` cross-tab support.

### Message safety (v2.7.0)

Cross-tab sync is hardened against malformed/cross-origin noise on the channel:

- Each instance generates its **origin ID** with `crypto.randomUUID()`, so a tab
  reliably ignores its own echoed messages.
- **Inbound `BroadcastChannel` messages are validated** (shape-checked) before
  they're applied, so a malformed or unexpected payload can't corrupt local
  state.
