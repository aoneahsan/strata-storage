...
> **Experimental:** `MigrationManager` is exported from `strata-storage`, but it is a standalone, adapter-level utility — it is **not** configured through `StrataConfig` (there is no `migrations` option on `new Strata(...)`). It operates directly on an initialized storage adapter.
